    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                half4 colR = tex2D(_MainTex, uv + _ColorDirection * _ColorRadius * 0.01);
                half4 colG = tex2D(_MainTex, uv - _ColorDirection * _ColorRadius * 0.01);
                float treshold = 0.5f;             
                float luminosity = 0.5;
                col.rgb += colR.rgb * step(_ColorRadius, -treshold) * luminosity;
                col.rgb += colG.gbr * step(0, _ColorRadius) * luminosity;
                return col;
            }
            ENDCG
        }
